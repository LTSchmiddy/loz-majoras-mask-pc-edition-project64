...
class GUI(BasePanel.BaseGUI):
    def __init__(self, rect, pos, jsonArgs = None, hostVars = None):
        ...
        self.buttons = {
            ...
            "Settings": AssetUI.LabelButton([80, 80], self.pos, "Emulation Settings", 30),
            "GFX": AssetUI.LabelButton([80, 120], self.pos, "Graphics Settings", 30),
            "Audio": AssetUI.LabelButton([80, 160], self.pos, "Audio Settings", 30),
            "Controls": AssetUI.LabelButton([80, 200], self.pos, "Controller Settings", 30),
            "Emu": AssetUI.LabelButton([80, 380], self.pos, "Open Emulator", 20),
        }
    ...
    def bg_tasks(self):
        self.show = (self.hostvar["currentMenu"] == "settings")
        ...
        for key, value in self.buttons.iteritems():
            if value.clickBool(event):
                ...
                if key == "Back":
                    ...
                    gsm.GameProc = subprocess.Popen([self.hostvar["mainSettings"]["Game"]["Game Exec"], "emusettings"], cwd=self.hostvar["mainSettings"]["Game"]["Game Working Dir"])

                elif key == "GFX":
                    self.updateGlobalTexturePath()
                    gsm.GameProc = subprocess.Popen([self.hostvar["mainSettings"]["Game"]["Game Exec"], "gfxsettings"], cwd=self.hostvar["mainSettings"]["Game"]["Game Working Dir"])

                elif key == "Audio":
                    gsm.GameProc = subprocess.Popen([self.hostvar["mainSettings"]["Game"]["Game Exec"], "audiosettings"], cwd=self.hostvar["mainSettings"]["Game"]["Game Working Dir"])

                elif key == "Controls":
                    gsm.GameProc = subprocess.Popen([self.hostvar["mainSettings"]["Game"]["Game Exec"], "controlsettings"], cwd=self.hostvar["mainSettings"]["Game"]["Game Working Dir"])

                elif key == "Emu":
                    gsm.GameProc = subprocess.Popen([self.hostvar["mainSettings"]["Game"]["Game Exec"], "showgui"], cwd=self.hostvar["mainSettings"]["Game"]["Game Working Dir"])
